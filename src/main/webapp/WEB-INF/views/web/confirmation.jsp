...
<section class="banner-area organic-breadcrumb">
	<div class="container">
		<div
			...
			<div class="col-first"><%@ taglib prefix="fmt"
...
<section class="order_details section_gap">
	<c:forEach var="order" items="${ orders }">
		<div class="container mb-5 p-4 border rounded shadow-sm">
			<c:if test="${order.orderStatus eq 'PENDING'}">
				<h3 class="title_pending">Your order is waiting to be
					packed.</h3>
			</c:if>
			<c:if test="${order.orderStatus eq 'COMPLETE'}">
				<h3 class="title_confirmation">Thank you. Your order has been
					received.</h3>
			</c:if>
			<c:if test="${order.orderStatus eq 'DELIVERING'}">
				<h3 class="title_delivering">Your order has been shipping.</h3>
			</c:if>
			<div class="row order_d_inner" style="position: relative;">
				...
				<span
					style="width: 20px; display: block; position: absolute; right: 0; cursor: pointer;"
					onclick="openOrderDetail(${order.id});"><img
					src="/FourT-Shop-E-commerce/asset/web/img/features/down-arrow.png">
				</span>
			</div>
			<div class="order_details_table" id="order_detail_${ order.id }"
				style="display: none;">
				<h2>Order Details</h2>
					...
					<table class="table">
						...
						<tbody>
									...
									<td>
										<p>
											<fmt:formatNumber type="currency"
												value="${orderDetail.price}" currencySymbol="$" />
										</p>
									</td>
								</tr>
							</c:forEach>
							<tr>
								...
								<td>
									<p>
										<fmt:formatNumber type="currency" value="${order.sumMoney}"
											currencySymbol="$" />
									</p>
								</td>
							</tr>
